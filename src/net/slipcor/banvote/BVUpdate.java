...
public class BVUpdate {
	...
	public static void updateCheck(Plugin p) {
	    String pluginUrlString = "http://dev.bukkit.org/server-mods/banvote/files.rss";
	    instance = p;
	    try {
	      ...
	      if (firstNode.getNodeType() == 1) {
	        ...
	        BVUpdate.vOnline = sOnlineVersion.replace("v", "");
	        BVUpdate.vThis = sThisVersion.replace("v", "");
	...
	public static void message(Player player) {
		...
		if (outdated) {
			BanVotePlugin.msg(player, "You are using "+colorize("v"+vThis,'o')+", an outdated version! Latest: "+colorize("v"+vOnline,'s'));
		} else {
			BanVotePlugin.msg(player, "You are using "+colorize("v"+vThis,'e')+", an experimental version! Latest stable: "+colorize("v"+vOnline,'s'));
		}
	}
